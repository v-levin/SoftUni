...
public class NationsBuilder
{
    ...
    private Bender GetBender(List<string> benderArgs)
    {
        ...
        var benderAuxParam = float.Parse(benderArgs[3]);
    ...
    private Monument GetMonument(List<string> monumentArgs)
    {
        ...
        var monumentAuxParam = int.Parse(monumentArgs[2]);

        switch (monumentType)
        {
            case "Air":
                return new AirMonument(monumentName, monumentAuxParam);
            case "Water":
                return new WaterMonument(monumentName, monumentAuxParam);
            case "Fire":
                return new FireMonument(monumentName, monumentAuxParam);
            case "Earth":
                return new EarthMonument(monumentName, monumentAuxParam);
            default:
                throw new ArgumentException();
        }
    }
